...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "The rebuild refuses when another MSACCESS.EXE in the Windows session holds one of "
        "the files it replaces, and never closes another Access process. It checks the loaded "
        "VBA projects in each one, so an instance with an unrelated database open does not "
        "block it; an instance that cannot be asked does, since a busy instance cannot be "
